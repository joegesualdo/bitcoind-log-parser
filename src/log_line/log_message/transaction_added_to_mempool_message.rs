...
impl TransactionAddedToMempoolMessage {
    pub fn is_transaction_added_to_mempool_log_line(message: &str) -> bool {
        return message.starts_with("TransactionAddedToMempool:");
    }

    pub fn parse(message: &str) -> Option<TransactionAddedToMempoolMessage> {
        if !Self::is_transaction_added_to_mempool_log_line(message) {
            return None;
        }
        ...
        Some(TransactionAddedToMempoolMessage {
            txid: txid.to_string(),
...
mod tests {
    ...
    fn is_transaction_added_to_mempool_log_line_fn_works() {
        ...
        let is_tatmp = TransactionAddedToMempoolMessage::is_transaction_added_to_mempool_log_line(
            raw_tatmp_message,
        );
